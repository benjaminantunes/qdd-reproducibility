...
!          for theta = 90, one has the maximum number of phis
      ...
      INTEGER :: kpoints=92
      INTEGER, PARAMETER :: komega=10000   ! max. nr. of frequencies in PES
...
!      parameter(name='H2O')                         ! set up the name
      ...
      REAL(8), ALLOCATABLE :: w(:),dir(:,:),dire(:,:),pes(:)
      REAL(8), ALLOCATABLE :: dist(:),pessave(:),thetz(:)
      INTEGER, ALLOCATABLE :: ksave(:),itheq(:)
     ...
     &          beta4(komega),beta6(komega)
      ...
      OPEN(unit=150,status='unknown',file='pPES.'//TRIM(name))
      READ(150,'(1x)')
      READ(150,'(2x,2i6)') kpoints
      REWIND(150)

      ALLOCATE(w(kpoints),dir(3,kpoints),dire(3,kpoints),pes(kpoints))
      ALLOCATE(dist(kpoints))
      ALLOCATE(ksave(kpoints),thetz(kpoints),itheq(kpoints))
      ALLOCATE(pessave(kpoints))
